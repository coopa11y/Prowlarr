...
## Toolbars And Status Feedback

- Use `ActionGroup` when several icon actions sit next to each other.
- Pass `actionLabel` and `context` to icon-only actions so NVDA announces each
  action separately.
- Use live regions for async test actions so pass/fail feedback is announced
  without forcing focus to move.
